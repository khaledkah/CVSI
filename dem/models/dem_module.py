...
from .components.cvsi import _LOG_W_FLOOR, idem_score_cvsi_fn
...
class DEMLitModule(LightningModule):
    """`LightningModule` implementing iterated Denoising Energy Matching.
    ...
    def __init__(
        ...
        nll_batch_size=256,
        use_cvsi=False,
        cvsi_weighted_stats=False,
        ct_per_molecule: bool = True,
        clip_all=False,
        time_range: float = 1.0,
    ) -> None:
        """Initialize a `DEMLitModule`.
        ...
        # The training/sampling pipeline is VE-only (a(t)=1): the forward noising, the
        # IS proposal, TSI/DSI, VEReverseSDE and the prior all assume it.
        assert not hasattr(noise_schedule, "a"), (
            "DEMLitModule assumes a VE noise schedule (a(t)=1); the provided "
            "schedule defines a scaling a(t)."
        )

        # Diffusion time interval is [0, time_range]; the reverse SDE starts
        # at t = time_range and the prior scale is derived from h(time_range)
        # in setup(), so changing the starting noise level stays consistent.
        self.time_range = time_range

        self.reverse_sde = VEReverseSDE(self.net, self.noise_schedule)

        if use_cvsi:
            self.clipper_tsi = clipper
            if clip_all:
                self.clipper = clipper
            else:
                self.clipper = None #clipper
        else:
            self.clipper = clipper
        ...
        grad_fxn = (
            ...
            if not use_cvsi
                ...
                idem_score_cvsi_fn,
                weighted_stats=cvsi_weighted_stats,
                ct_per_molecule=ct_per_molecule,
                debug=False,
                clipper_tsi=self.clipper_tsi,
            )
        )
        ...
        if use_cvsi:
            if clip_all:
                ...
                self.clipped_grad_fxn = grad_fxn
        else:
            self.clipped_grad_fxn = self.clipper.wrap_grad_fxn(grad_fxn)
        ...
        self.test_big_batch_logz = MeanMetric()
        ...
        self.test_big_batch_ess = MeanMetric()
        ...
        self.use_cvsi = use_cvsi
        self.cvsi_weighted_stats = cvsi_weighted_stats
        self.ct_per_molecule = ct_per_molecule
    ...
    def _log_cvsi_diagnostics(self, times, tilde_ct, tsi_broad, prefix):
        """Log the CVSI interpolation coefficient: ~0 relies on TSI, ~1 on DSI."""
        if getattr(self, "_trainer", None) is None:
            return

        with torch.no_grad():
            # Reduce tilde_ct (whatever broadcast shape it has) to one value per batch element.
            tct = (tilde_ct * torch.ones_like(tsi_broad)).mean(dim=-1)
            tct = tct.reshape(-1, times.shape[0]).mean(dim=0)

            is_train = prefix == "train"
            self.log(
                f"{prefix}/cvsi/tilde_ct", tct.mean(), on_step=is_train, on_epoch=True
            )
            self.log_dict(
                t_stratified_loss(times, tct, loss_name=f"{prefix}/stratified/tilde_ct"),
                on_step=is_train,
                on_epoch=True,
            )

    def get_loss(
        self, times: torch.Tensor, samples: torch.Tensor, prefix: str = "train"
    ) -> torch.Tensor:
        if self.use_cvsi:
            grad_fxn = partial(
                idem_score_cvsi_fn,
                weighted_stats=self.cvsi_weighted_stats,
                ct_per_molecule=self.ct_per_molecule,
                debug=True,
                clipper_tsi=self.clipper_tsi,
            )
        else:
        ...
        if self.use_cvsi and isinstance(estimated_score, tuple):
            (
                estimated_score,
                w,
                tilde_ct,
            ...
            self._log_cvsi_diagnostics(times, tilde_ct, tsi_broad, prefix)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.hparams.debug_use_train_data:
            ...
            dem_loss = self.get_loss(times, noised_samples, prefix="train")
            # Uncomment for SM
    ...
    def on_train_epoch_end(self) -> None:
        ...
        self._log_nll_against_model(prefix="val")

        if self.energy_function.is_molecule:
            self._log_dist_w2(prefix="val")
            self._log_dist_total_var(prefix="val")

    def _log_nll_against_model(self, prefix="val"):
        """
        Compute the NLL of generated samples coming from the True target model.
        """
        if prefix == "test":
        ...
        self.log(
            f"{prefix}/nll_against_model",
            self.val_nll_model(-generated_energies.mean()),
            on_step=False,
            on_epoch=True,
            prog_bar=True,
        )
    ...
    def _log_dist_w2(self, prefix="val"):
        ...
        # Unnormalize generated_samples to match the physical space of data_set (buffer stores normalized)
    ...
    def eval_step(self, prefix: str, batch: torch.Tensor, batch_idx: int) -> None:
        ...
        times = (
            torch.rand((self.eval_batch_size,), device=batch.device) * self.time_range
        )

        normalized_batch = self.energy_function.normalize(batch)
        noised_batch = normalized_batch + (
            torch.randn_like(normalized_batch)
            * self.noise_schedule.h(times).sqrt().unsqueeze(-1)
        )
        ...
        loss = self.get_loss(times, noised_batch, prefix=prefix).mean(-1)
        ...
        if self.nll_with_dem:
            ...
            self.compute_log_z(
                self.dem_cnf, self.prior, backwards_samples, prefix, "dem_"
            )
        if self.nll_with_cfm:
            forwards_samples = self.compute_and_log_nll(
    ...
    def on_test_epoch_end(self) -> None:
        ...
        self._log_nll_against_model(prefix="test")
        if self.energy_function.is_molecule:
        ...
        if self.nll_with_dem:
            print("Computing big-batch NLL/logZ/ESS")
            self.compute_log_z(
                self.dem_cnf, self.prior, final_samples[idx], "test", "big_batch_"
            )
    ...
    def setup(self, stage: str) -> None:
        ...
        # Prior std matches the marginal at the reverse-process start time, so changing
        # time_range keeps sampling consistent. The VE prior N(0, h(t)) ~ q_t is only a
        # good approximation while h(time_range) >> Var(data).
        self.prior = self.partial_prior(
            device=self.device,
            scale=self.noise_schedule.h(
                torch.tensor(self.time_range, device=self.device)
            )
            ** 0.5,
        )
