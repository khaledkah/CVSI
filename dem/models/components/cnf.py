...
class CNF(torch.nn.Module):
    def __init__(
        ...
        # End point of the diffusion time interval [0, time_range]; must match
        # the reverse-SDE start time and the prior scale (h(time_range)).
        # Only used when is_diffusion=True; CFM time is always [0, 1].
    ...
    def integrate(self, x):
        method = self.method
        # diffusion: data (t=0) -> prior (t=time_range); CFM: 1 -> 0
        end_time = self.time_range if self.is_diffusion else 0.0
        start_time = 0.0 if self.is_diffusion else 1.0
    ...
    def generate(self, x):
        ...
        # diffusion: prior (t=time_range) -> data (t=0); CFM: 0 -> 1
        start_time = self.time_range if self.is_diffusion else 0.0
        end_time = 0.0 if self.is_diffusion else 1.0
