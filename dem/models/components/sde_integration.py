...
def integrate_sde(
    ...
    # start_time/end_time override the default VE domain. Defaults preserve VE
    # exactly (start=time_range, end=0 for reverse). For VP the reverse SDE runs
    # over the schedule's native domain, e.g. start=t_max, end=t_min.
    if start_time is None:
        start_time = time_range if reverse_time else 0.0
    if end_time is None:
        end_time = time_range - (time_range if reverse_time else 0.0)

    times = torch.linspace(
        start_time, end_time, num_integration_steps + 1, device=x0.device
    )[:-1]
    # dt is the magnitude of each step in the integration variable; equals
    # time_range/N for the default [time_range, 0] VE domain.
    dt = abs(start_time - end_time) / num_integration_steps
